...
public class DbgOntProcessor extends Thread {
    ...
    private long   initialRecordCount = 0;
    private String orgProfile = "0";
    private String continueProcessing = "NO";
				            ...
				            String continueProcessing) 
	{
	   ...
	   try
	   {
		  if( nextRecord.equals("0"))
		  {
			  osd = new PrintWriter( new FileWriter(outputFile));         // create new output data file
		      ...
		      recordCount = 0; // initialise record count to zero - this value will be increased by 1 and put with each new record
		      
		      initialRecordCount = 0;
		  }
		  else
		  {
			  osd = new PrintWriter( new FileWriter(outputFile,true));    // append to existing output data file and do NOT write header
			  
			  recordCount = new Long(nextRecord).longValue() - 1;         // initialise record count to nextRecord-1 - this value will be increased by 1 and put with each new record
			  
			  initialRecordCount = recordCount;  // save already existing number of emplyee records
		  }
	...
	public void run()
	{
        ...
        for (Element listing : listings){
            ...
            for (Element branch : branches){
            	...
            	if( orgProfile.equals("0") || (!orgProfile.equals("0") && orgProfile.equals(branchCode)) )
            	{
	                ...
	                if( continueProcessing.equals("YES") && (!orgProfile.equals("0") && orgProfile.equals(branchCode)) )
	                {
	                	orgProfile = "0";  // reset orgProfile so FROM THIS POINT all organisations will be processed
	                }
	                else
	                {
	                	// only this organisation will be processed
	                }
            	}
            }
            System.out.println();
        ...
        System.out.println("\n\n[" + now.getTime() + "] Program exiting, pages processed=" + pageCount + ", records processed=" + (recordCount-initialRecordCount) );
